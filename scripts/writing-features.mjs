...
export const FEATURES = [
  /* ---- AI as the material -------------------------------------------- */
  {
    ...
    alt: 'A large sage boom barrier drawn flat from the side, its post at the left, its arm raised at forty-five degrees and running off the top of the frame, and a solid black counterweight on the arm\u2019s short tail.',
    prompt: `Same style, same hand as the reference. Cream ground. One large sage green boom barrier, the kind at a rail crossing or a parking garage, drawn as a strict flat side elevation, the way a technical diagram is drawn: the whole picture is one flat plane seen exactly from the side, with no vanishing point, no foreshortening, no ellipses, no ground plane and no depth of any kind. A plain thick post stands near the left edge, a tall rectangle with a rounded top, standing on the bottom edge of the frame and cropped by it. On the face of the post, one plain round pivot, a circle with a black outline. From the pivot, the arm: one long thick straight bar of even width, sage with a black outline, rising from the pivot at about forty-five degrees up and off the top-right corner of the frame. The same bar continues a short way past the pivot, down and to the left, as its tail, and on the end of the tail sits the counterweight, one solid black flat block with rounded corners, the only black shape in the picture. Nothing is passing under the arm; the ground below it is empty cream. No shadow, no road, no people, no cars, no stripes on the arm. Nothing else in the picture.`,
  },
  {
