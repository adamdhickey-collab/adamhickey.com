...
**Writing** -- `writing/index.html` and the ten articles beside it. No fact
...
| `img/writing/` | Two drawings per article, in two styles: the feature, an isometric diagram of the argument, stored as the 1600x900 feature under the article's dek and the 640x360 card beside its entry on the index, both cut from one source through `scripts/illustrate.mjs feature` and `card`; and `<slug>-2.webp`, the argument's second half in the engagement set's painterly style, at 1600x900 in the body, drawn from `scripts/writing-scenes.mjs` through `scripts/draw.mjs`. The features run far brighter than the engagement set's 126-184 mean (233-249) and `illustrate.mjs` flags every one of them; that is the diagram style, not a bad exposure, and the band still governs the painterly drawings |
