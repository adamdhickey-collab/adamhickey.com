...
| `img/engagement/` | The four card illustrations at 1080x720, each engagement page's hero and invitation, and the numbered step drawings. **The four card illustrations are no longer in this folder's hand.** `01-clarity`, `02-system`, `03-embedded` and `04-ai` are the homepage's "How I work" grid, and they were redrawn in the writing set's hand -- flat vector, one everyday object drawn whole with a narrow band of ground around it and only its black ribbon leaving the frame, black as a solid shape, no paper grain -- from `scripts/proof-cards.mjs` through `scripts/draw.mjs --set proof`, cut to this folder's 3:2 by `illustrate.mjs step` rather than the writing set's 16:9. They keep their paths because only the homepage loads them. Everything else in the folder is still the engagement spec below. The retired brand page's drawings stay, as part of the set. The Whole Thing's six went to the archive on 2026-09-08: no page here loads them, they were the one set never solved to its hero, and `adamhickey-next` holds them at the bytes they left with |
