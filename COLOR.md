...
`style.css` holds no shared token at all. It is loaded by twenty-three pages; the
token files are loaded by all twenty-nine, so anything defined in `style.css` is
